...
| [`crater/fiber_traces.md`](crater/fiber_traces.md), [`crater/verify_traces.py`](crater/verify_traces.py) | **The centroid is calm; the variance sees the crater**: all first coordinate traces are polynomial and five of six quadratic traces are too, but **Tr(x²)=2(3BC−4)/L** has a pole where the paired sheets escape. The power-basis trace form has determinant **−4S²/L³**: its zero is the phantom S-wall where x stops separating sheets; its pole is the nonproperness rim. |
